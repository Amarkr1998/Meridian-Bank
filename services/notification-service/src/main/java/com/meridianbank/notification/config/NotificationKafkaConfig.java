...
/**
 * This project's second genuine Kafka <em>consumer</em>, after {@code audit-service} (Phase 11) —
 * see docs/architecture/kafka-architecture.md's "Idempotent consumers" / "Retry + DLQ" principles,
 * which describe exactly the policy wired here. Backs every {@code @KafkaListener} in
 * {@link com.meridianbank.notification.ingest.NotificationEventListener}, one per consumed topic
 * (`payment.completed`, `payment.failed`, `kyc.updated`, `account.approved`, `fraud.detected`,
 * `notification.requested`), all under one shared consumer group ({@code notification-service} —
 * see application.yml). A message that fails processing is retried with exponential backoff up to
 * {@code meridian.notification-consumer.max-attempts} total attempts; once exhausted, the raw
 * record is published as-is to {@code <topic>.DLQ} by {@link DeadLetterPublishingRecoverer} and
 * the original offset is committed — a permanently unprocessable message does not block the
 * partition forever. This mirrors the producer-side retry/DLQ policy every outbox-using service
 * already implements (see any of their {@code OutboxPublisher} classes) — same shape, opposite
 * side of the topic. See notification-service/README.md for exactly which topics are consumed.
 */
@Configuration
