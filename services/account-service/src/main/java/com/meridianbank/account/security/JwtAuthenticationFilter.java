...
/**
 * See auth-service's JwtAuthenticationFilter — identical pattern, verifying tokens issued there.
 * Also stashes the raw bearer token as a request attribute: CustomerKycServiceClient forwards it
 * when checking a customer's KYC status, since that check is itself resource-owned
 * ({@code #id == authentication.principal}) on the customer-kyc-service side.
 */
@Component
