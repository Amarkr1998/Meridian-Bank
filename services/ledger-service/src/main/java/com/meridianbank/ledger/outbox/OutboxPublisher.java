 ...
 * consume the `audit.event` this service now also publishes for reconciliation mismatches and
 * resolutions (see its `audit` package) — but this service's own domain event
 * (`reconciliation.completed`) still has no consumer; `notification-service` remains a future
 * phase.
 *
